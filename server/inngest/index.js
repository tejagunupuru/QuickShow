...
const releaseSeatsAndDeleteBooking = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("check-payment-status", async () => {
      ...
      //if payment is not made release seats
      if (!booking.isPaid) {
...
<h2>Hi ${
        booking.user.name
      },</h2> <p>Your booking for <strong style="color: #F84565;">"${
...
const sendShowRemainders = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const results = await step.run("send-all-remianders", async () => {
...
<p>Enjoy the show!<br/>QuickShow Team</p>
    ...
    return {
      sent,
      failed,
      message: `${sent} remainder(s),${failed} failed`,
    };
  }
);
//Innegest function to send notification when a new show is added
