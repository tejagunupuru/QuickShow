...
const AddShows = () => {
  ...
  const fetchNowPlayingMovies = async () => {
    try {
      const { data } = await axios.get(
        "/api/show/now-playing?title=avengers", // Dynamic later if needed
        {
  ...
  const handleDateTimeAdd = () => {
    if (!dateTimeInput.includes("T")) return;
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      <button
        ...
        disabled={addingShow}
        className="bg-primary text-white px-8 py-2 mt-6 hover:bg-primary/90 transition-all cursor-pointer rounded-md"
      >
